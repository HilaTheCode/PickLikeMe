...
class AssignBurstSplitsTests(unittest.TestCase):
    def test_bursts_are_never_split_across_train_and_test(self):
        result = assign_burst_splits(_labels_frame(), test_fraction=0.3, seed=7)
        for _, group in result[result["burst_id"].notna()].groupby("burst_id"):
            self.assertEqual(len(set(group["split"])), 1)

    def test_deterministic_for_same_seed(self):
        first = assign_burst_splits(_labels_frame(), test_fraction=0.3, seed=7)
        second = assign_burst_splits(_labels_frame(), test_fraction=0.3, seed=7)
        self.assertTrue(first.equals(second))

    def test_different_seed_changes_assignment(self):
        first = assign_burst_splits(_labels_frame(), test_fraction=0.3, seed=7)
        second = assign_burst_splits(_labels_frame(), test_fraction=0.3, seed=8)
...
class CreateSplitTests(unittest.TestCase):
    def test_refuses_to_overwrite_existing_split(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _labels_frame().to_csv(labels_path, index=False)

            create_split(labels_path, split_path)
            with self.assertRaises(FileExistsError):
                create_split(labels_path, split_path)
            create_split(labels_path, split_path, force=True)
    ...
    def test_load_split_round_trip(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            created = create_split(labels_path, split_path)
