...
## Follow-up: architectural review of Open Folder (post-sprint)

Even after Priority 1, "Loading categories…" kept visibly ticking for the full duration of a large folder's background metadata pass, and background work could survive the window closing. A second pass audited the *architecture*, not just the symptom, comparing against `review/page.py`/`server.py` (the reference implementation) line by line rather than assuming prior conclusions still held.

**Finding:** `page.py` never polls for completion at all — no `setInterval` anywhere tied to loading. `switchFolder()` fires one request, renders once, and stops; the "Loading categories…" status line is set exactly once, from that single response, and is never refreshed again. The background daemon thread (`ReviewSession._background_load`, shared by both apps) keeps mutating `session.images` in place regardless; the *next* unrelated interaction's own `as_dict()` call incidentally reveals whatever finished by then. Desktop, by contrast, had invented a `QTimer` on a 200ms interval that ran for the entire background pass, continuously repainting a status-bar progress bar and re-writing status text — a mechanism with no Web equivalent at all, built to drive a modal dialog that (post-Priority-1) closes immediately and can never be visible while the timer ticks. Two of the methods it called every tick (`_update_folder_load_dialog`/`_update_folder_load_dialog_progress`) were consequently dead code, no-oping on a `None` dialog reference on every single call.

**Consumer audit of `detected_category`** (every read, not just the loading path): Web's `page.py` uses it for a real, working category-filter dropdown and a per-card badge chip (`page.py:344,355,600-601`). Desktop uses it for nothing except a diagnostic count in the now-removed dialog detail text. `captured_at` has one real Desktop consumer (`SORT_FIELDS`'s "Capture Time" sort) and one real Web consumer (the `date` sort key) — neither needed before the default (score-sorted) view renders. Both fields are computed together in one shared per-image loop in `ReviewSession._background_load`, so the phase itself couldn't be deleted wholesale without breaking Web's category filter — but Desktop's *narration* of that phase as a trackable, ongoing operation had zero Desktop consumers and was pure invented overhead.

**Bonus finding, unrelated, left unfixed:** `review/server.py`'s `_serve_crop_data` imports `detected_category_for` from `..thumbnails` — which resolves to `picklikeme.thumbnails`, a module that doesn't exist (only `picklikeme.review.thumbnails` does). Confirmed via direct import that this raises `ModuleNotFoundError`. The import (along with two others) is also never actually used anywhere in that function body. Out of scope for this pass; flagged for a separate fix.

**Redesign, matching Web exactly:**
- Deleted `_loading_timer`, `_poll_loading_state`, `_update_folder_load_dialog`, `_update_folder_load_dialog_progress`, the `_loading_progress` status-bar widget, and the now-unused `_background_load_active`/`_thumbnail_generation_count`/`_metadata_loaded_count`/`_last_loading_stage` state — no replacement polling. `QTimer`/`QProgressBar` dropped from imports as a result.
- `_start_open_folder` still shows one status message once (complete vs. still-loading in the background) — unchanged, matches Web's one-shot status line exactly.
- Desktop already had Web's "next interaction refreshes full state" property throughout (`apply_review_status`, rank, organize, auto-crop, cutoff — all already call `service.load_session()` fresh) — this needed no new code, just recognizing it was already sufficient once the timer stopped assuming it wasn't.
- Added exactly one on-demand, lazy refresh: switching the sort field to "Capture Time" — the one real Desktop consumer of the background data — triggers a single `load_session()` call at that moment, not a poll, matching "load lazily when first required."
- `closeEvent` now bumps `self.service.session._loading_generation` before closing. `_background_load` already checks this counter every 25 images and bails out on a mismatch (the same mechanism that already stopped it on reopening a folder) — extending it to app close was a one-line fix, not a new mechanism.

**Verified, not assumed:** an offscreen repro with a monkeypatched slow `detected_category_for` (100 images, artificially delayed) confirmed `open_folder()` returns in 0.17s while the backend is still at 20%/`loading-categories`/`complete: False`; `window.close()` returns in 0.000s; and — the actual regression — the backend's `loading_state()` is still frozen at the same 20% a full second *after* close, proving the background thread genuinely stopped rather than continuing invisibly. Full regression suite (desktop + `test_review_session.py` + `test_review_server.py`, 257 tests) passes unchanged, confirming Web's shared-backend behavior was never touched.
