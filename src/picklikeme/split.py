...
singleton bursts. Assignment is deterministic for a given (labels, seed,
...
def assign_burst_splits(
    ...
    if "image_path" not in labels.columns:
        raise ValueError("labels frame must contain an image_path column")
...
def create_split(
    ...
    split = assign_burst_splits(labels, test_fraction=test_fraction, seed=seed)
...
def main() -> None:
    ...
    parser.add_argument("--labels", default="data/labels.csv", help="Labels CSV with image_path,label,burst_id")
